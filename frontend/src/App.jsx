...
function App() {
    ...
    fetch('http://127.0.0.1:8000/api/leadership/')
      ...
      .then(data => {
        if (data && data.length > 0) {
          ...
          setLeadershipData(formattedLeadership);
        }
      })
  ...
  return (
    <div className="relative min-h-screen bg-black text-[#ededed] overflow-x-hidden selection:bg-white/20">
      ...
      {/* Full Width Navbar */}
      <header className={`fixed top-0 left-0 right-0 z-[100] w-full transition-all duration-300 ${scrolled ? 'bg-black/70 backdrop-blur-xl border-b border-white/10 shadow-2xl py-2' : 'bg-transparent py-2'}`}>
            ...
            {logoUrl ? (
              <div className="relative flex items-center">
                <button onClick={() => setViewingLogo(true)} className="focus:outline-none transition-transform hover:scale-105">
                  <img src={logoUrl} alt="Club Logo" className="h-14 w-14 rounded-full object-cover border-2 border-white/10 drop-shadow-md bg-black" />
                </button>
              </div>
            ) : (
              <a href="#home" className="flex items-center gap-2">
                <span className="w-10 h-10 rounded-full bg-gradient-to-br from-white to-gray-400 text-black flex items-center justify-center font-bold text-lg shadow-[0_0_20px_rgba(255,255,255,0.2)]">C</span>
                <span className="font-bold tracking-tight text-white text-xl">Codician</span>
              </a>
            )}
          </div>

        <nav className={`${menuOpen ? 'flex absolute top-[80px] left-0 right-0 flex-col p-6 rounded-3xl bg-black/80 backdrop-blur-2xl border border-white/10 shadow-2xl' : 'hidden'} md:flex md:static md:flex-row md:bg-transparent md:border-none md:p-0 gap-6 items-center`}>
          {['About', 'College', 'Team', 'Domains', 'Events', 'Contact'].map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} className="text-sm text-gray-400 hover:text-white transition-colors max-md:py-3 max-md:text-lg max-md:w-full max-md:border-b max-md:border-white/5" onClick={() => setMenuOpen(false)}>
              {item}
            </a>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-4">
          <button className="hidden md:block bg-white text-black text-sm font-semibold px-5 py-2 rounded-full shimmer-btn hover:scale-105 transition-transform duration-300" onClick={() => openRegistration("the Club")}>
            Join the Club
          </button>
          
          <button className="md:hidden text-white p-2" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle Navigation">
            {menuOpen ? '✕' : '☰'}
          </button>
        </div>
        </div>
      ...
      <main className="relative z-10 flex flex-col items-center w-full max-w-7xl mx-auto px-6 pt-32">
        ...
        <section id="home" className="min-h-[85vh] w-full flex flex-col justify-center items-center text-center mt-12 mb-16 animate-fade-in-up relative">
          ...
          {logoUrl ? (
             <div className="mb-8 md:mb-12 w-48 h-48 md:w-64 md:h-64 rounded-full overflow-hidden border-4 border-white/10 shadow-[0_0_40px_rgba(255,255,255,0.15)] bg-black/50 flex items-center justify-center mx-auto">
             ...
             <div className="mb-8 md:mb-12 w-48 h-48 md:w-64 md:h-64 rounded-full border border-white/20 flex items-center justify-center bg-gradient-to-br from-white/10 to-transparent backdrop-blur-md shadow-[0_0_30px_rgba(255,255,255,0.1)]">
               <span className="text-7xl font-bold text-white opacity-80">C</span>
             </div>
          )}

          <h1 className="text-[clamp(2.5rem,6vw,5.5rem)] font-black uppercase leading-[0.9] tracking-tighter mb-6 text-transparent bg-clip-text bg-gradient-to-b from-white via-gray-200 to-gray-500" style={{ filter: 'drop-shadow(0px 10px 20px rgba(0,0,0,0.5))' }}>
            CODICIAN CLUB
          </h1>
          
          <p className="text-base md:text-xl text-gray-300 font-medium tracking-[0.2em] md:tracking-[0.3em] mb-16 uppercase px-4 text-shadow-sm">
            "WHERE CODERS BECOME CREATORS."
          </p>

          <div className="flex flex-col sm:flex-row gap-4 justify-center z-20">
            <a href="#about" className="bg-white text-black font-bold uppercase tracking-widest text-sm px-10 py-4 rounded-full shimmer-btn hover:scale-105 transition-transform duration-300 shadow-[0_0_30px_rgba(255,255,255,0.15)] flex items-center justify-center gap-2">
              Explore
        ...
        <section id="about" className="w-full scroll-mt-32 mb-32 text-center">
          <div className="mb-12 border-b border-white/10 pb-8">
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-4 text-white">About Us</h2>
          </div>
          ...
          <div className="p-10 md:p-16 rounded-3xl premium-glass shadow-2xl relative overflow-hidden group hover:-translate-y-2 transition-transform duration-500 text-left">
             ...
             <p className="text-xl md:text-2xl text-gray-300 leading-relaxed font-medium relative z-10">
               {aboutUsText}
             </p>
          </div>
        </section>
        ...
        <section id="college" className="w-full scroll-mt-32 mb-32 text-center">
          <div className="mb-12 border-b border-white/10 pb-8">
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-4 text-white">Our College</h2>
          </div>
          ...
          <div className="p-8 md:p-12 rounded-3xl premium-glass shadow-2xl relative overflow-hidden group hover:-translate-y-2 transition-transform duration-500 flex flex-col md:flex-row items-center gap-8 md:gap-12 text-left">
             ...
             <div className="flex-shrink-0 relative z-10 w-40 h-40 md:w-56 md:h-56 rounded-full overflow-hidden border-4 border-white/10 shadow-[0_0_30px_rgba(255,255,255,0.1)] group-hover:scale-105 group-hover:border-white/30 transition-all duration-500">
             ...
             <div className="relative z-10 flex flex-col text-center md:text-left">
                <h3 className="text-3xl md:text-4xl font-extrabold text-white mb-4 tracking-tight group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-blue-300 transition-all duration-300">
                  {collegeData.name}
                </h3>
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-3">
                  {collegeData.location && (
                    <a 
                      ...
                      className="group flex items-center gap-2 px-5 py-2.5 rounded-full border border-white/20 bg-white/5 hover:bg-emerald-500/20 hover:border-emerald-500/50 text-sm font-semibold text-gray-300 hover:text-emerald-400 backdrop-blur-md transition-all duration-300"
                    >
        ...
        <section id="domains" className="w-full scroll-mt-32 mb-32 text-center">
          <div className="mb-16 border-b border-white/10 pb-8 flex flex-col items-center">
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-4 text-white">Our Domains</h2>
            <p className="text-gray-400 text-lg max-w-2xl">We specialize in mastering the core pillars of modern engineering.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-left">
            {domainsData.map((domain, i) => (
              <div key={i} className="group flex flex-col p-8 rounded-3xl premium-glass hover:-translate-y-2 cursor-pointer">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-white/10 to-transparent flex items-center justify-center text-3xl mb-8 border border-white/10 shadow-[0_0_15px_rgba(255,255,255,0.05)] group-hover:scale-110 group-hover:bg-white/20 transition-all duration-500 overflow-hidden">
                  {domain.image ? (
                ...
                <h3 className="text-2xl font-bold text-gray-100 mb-3 tracking-tight">{domain.title}</h3>
                <p className="text-gray-400 leading-relaxed text-sm font-medium">{domain.description}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="events" className="w-full scroll-mt-32 mb-32 text-center">
          <div className="border-b border-white/10 pb-8 mb-16 flex flex-col items-center">
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-4 text-white">Event Galleries</h2>
            <p className="text-gray-400 text-lg max-w-2xl">A collection of albums from our past meetups, hackathons, and masterclasses.</p>
          </div>
          ...
          <div className="relative flex flex-col w-full max-w-6xl mx-auto py-10 text-left">
            ...
            <div className="absolute left-8 md:hidden top-0 bottom-0 w-1 bg-white/10 rounded-full z-0"></div>
              ...
              return (
                <div key={i} className="relative flex items-center w-full min-h-[500px] mb-16 md:mb-0">
                  ...
                  <div className={`relative z-10 w-full pl-20 pr-4 md:px-0 md:w-[45%] ${isEven ? 'md:ml-auto md:mr-0' : 'md:mr-auto md:ml-0'}`}>
                     ...
                     {/* EVENT CARD */}
                     <div className="group flex flex-col rounded-[2.5rem] premium-glass hover:-translate-y-2 cursor-pointer overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.4)] border border-white/10" onClick={() => setActiveAlbum(event)}>
                       {/* Image Section */}
                       <div className="relative h-56 lg:h-72 w-full bg-black overflow-hidden flex items-center justify-center">
                         ...
                         <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                           {event.images.length > 0 ? (
                             ...
                             <div className="w-full h-full bg-white/5 flex items-center justify-center text-gray-500">No images</div>
                           )}
                         </div>
                       </div>
                       
                       {/* Content Section */}
                       <div className="p-6 lg:p-8 flex-1 flex flex-col justify-between bg-[#111] backdrop-blur-xl relative border-t border-white/5">
                         <div>
                           <h3 className="text-xl lg:text-2xl font-bold text-white mb-3 tracking-tight group-hover:text-emerald-400 transition-colors duration-300">{event.title}</h3>
                           <p className={`text-gray-400 text-sm font-medium leading-relaxed mb-4 transition-all duration-500 ${expandedEvents[i] ? '' : 'line-clamp-2'}`}>
                             {event.text}
                           </p>
                         ...
                         <div className="flex justify-end">
                           {event.images.length > 0 && (
                             <div className="inline-flex bg-white/5 hover:bg-white/10 backdrop-blur-xl px-4 py-2 lg:px-5 lg:py-3 rounded-full text-xs lg:text-sm font-bold text-white border border-white/10 items-center gap-2 transition-all duration-500 shadow-xl">
        ...
        <section id="leadership" className="w-full scroll-mt-32 mb-32">
          <div className="border-b border-white/10 pb-8 mb-16 flex flex-col items-center text-center">
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-4 text-white">Our Leadership</h2>
            <p className="text-gray-400 text-lg max-w-2xl">The visionaries steering the Codician Club towards a future of innovation and excellence.</p>
          </div>

          <div className="flex flex-wrap justify-center gap-8 text-center">
            {leadershipData.length > 0 ? (
              leadershipData.map((ldr, i) => (
                <div key={i} className="w-full md:w-[calc(50%-1rem)] lg:w-[calc(33.333%-1.5rem)] max-w-sm group flex flex-col items-center p-8 rounded-3xl premium-glass hover:-translate-y-2 cursor-pointer shadow-xl relative overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-b from-white/[0.02] to-transparent pointer-events-none"></div>
                  <div 
                    className="w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden border-4 border-white/10 group-hover:border-white/30 transition-all duration-500 mb-6 bg-black relative z-10 shadow-lg cursor-pointer flex items-center justify-center"
                  ...
                  <h3 className="text-2xl font-bold text-white group-hover:text-emerald-300 transition-colors mb-1 relative z-10">{ldr.name}</h3>
                  <span className="text-sm font-bold text-emerald-400 uppercase tracking-widest mb-4 relative z-10">{ldr.role}</span>
                  <p className="text-gray-400 text-sm leading-relaxed mb-6 relative z-10">{ldr.description}</p>
                  
                  {ldr.linkedin && (
                    <a href={ldr.linkedin} target="_blank" rel="noopener noreferrer" className="mt-auto inline-flex items-center gap-2 text-xs font-bold text-white bg-white/10 hover:bg-white hover:text-black px-4 py-2 rounded-full transition-all duration-300 border border-white/20 relative z-10">
              ...
              [
                ...
                <div key={i} className="w-full md:w-[calc(50%-1rem)] lg:w-[calc(33.333%-1.5rem)] max-w-sm group flex flex-col items-center p-8 rounded-3xl premium-glass hover:-translate-y-2 cursor-pointer shadow-xl relative overflow-hidden opacity-50 border-dashed border-2 border-white/20">
                  <div className="absolute inset-0 bg-gradient-to-b from-white/[0.02] to-transparent pointer-events-none"></div>
                  <div className="w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden border-4 border-white/10 group-hover:border-white/30 transition-all duration-500 mb-6 bg-black relative z-10 shadow-lg">
                    <div className="w-full h-full flex items-center justify-center text-gray-500 text-sm text-center px-4">Awaiting Profile</div>
                  </div>
                  <h3 className="text-2xl font-bold text-gray-400 transition-colors mb-1 relative z-10">{ldr.name}</h3>
        ...
        <section id="team" className="w-full scroll-mt-32 mb-16">
          <div className="border-b border-white/10 pb-8 mb-16 flex flex-col items-center text-center">
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-4 text-white">Core Team</h2>
            <p className="text-gray-400 text-lg max-w-2xl">The brilliant minds working tirelessly behind the scenes to make it all happen.</p>
          </div>
            ...
            {coreTeamData.length > 0 ? (
              coreTeamData.map((member, i) => (
                <div key={i} className="w-full md:w-[calc(50%-1rem)] lg:w-[calc(33.333%-1.5rem)] max-w-sm group flex flex-col items-center p-8 rounded-3xl premium-glass hover:-translate-y-2 cursor-pointer shadow-xl relative overflow-hidden">
                  ...
                  <h3 className="text-2xl font-bold text-white group-hover:text-emerald-300 transition-colors mb-1 relative z-10">{member.name}</h3>
                  <span className="text-sm font-bold text-emerald-400 uppercase tracking-widest mb-2 relative z-10">{member.position}</span>
                  <span className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-4 relative z-10">{member.course}, {member.semester}</span>
                  <p className="text-gray-400 text-sm leading-relaxed mb-6 relative z-10">{member.description}</p>
                  
                  {member.linkedin && (
                    <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="mt-auto inline-flex items-center gap-2 text-xs font-bold text-white bg-white/10 hover:bg-white hover:text-black px-4 py-2 rounded-full transition-all duration-300 border border-white/20 relative z-10">
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
                      LinkedIn
                    </a>
                  )}
              ...
              [
                ...
                <div key={i} className="w-full md:w-[calc(50%-1rem)] lg:w-[calc(33.333%-1.5rem)] max-w-sm group flex flex-col items-center p-8 rounded-3xl premium-glass hover:-translate-y-2 cursor-pointer shadow-xl relative overflow-hidden opacity-50 border-dashed border-2 border-white/20">
                  ...
                  <h3 className="text-2xl font-bold text-gray-400 transition-colors mb-1 relative z-10">{member.name}</h3>
        ...
        <section id="contact" className="w-full scroll-mt-32 mb-24 flex flex-col items-center">
          <div className="border-b border-white/10 pb-6 mb-6 w-full text-center">
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-white mb-4">Get in Touch</h2>
            <p className="text-gray-400 text-lg max-w-2xl mx-auto">We're always looking to connect with passionate developers.</p>
          </div>
          ...
          <div className="w-full max-w-3xl p-10 md:p-16 rounded-[2.5rem] premium-glass flex flex-col items-center text-center shadow-2xl relative overflow-hidden group">
            ...
            <div className="mb-10 flex flex-col items-center relative z-10 cursor-pointer" onClick={() => { if(logoUrl) setViewingLogo(true); }}>
              <div className="w-28 h-28 md:w-36 md:h-36 rounded-full overflow-hidden border-4 border-white/10 group-hover:border-white/30 transition-all duration-500 mb-6 bg-black shadow-xl relative flex items-center justify-center">
              ...
              <h3 className="text-3xl md:text-5xl font-extrabold text-white tracking-tight group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-emerald-200 transition-all duration-500">
                Codician Club
              </h3>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto relative z-10">
              <a href={`mailto:${socialLinks.email}`} className="flex items-center justify-center gap-2 px-8 py-4 rounded-full bg-white/5 border border-white/10 hover:bg-white hover:text-black hover:scale-105 text-gray-300 transition-all duration-300 text-sm font-semibold shadow-lg">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                Email Us
              </a>
              <a href={socialLinks.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 px-8 py-4 rounded-full bg-white/5 border border-white/10 hover:bg-[#0077b5]/90 hover:border-transparent hover:scale-105 text-gray-300 hover:text-white transition-all duration-300 text-sm font-semibold shadow-lg">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
                LinkedIn
              </a>
              <a href={socialLinks.instagram} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 px-8 py-4 rounded-full bg-white/5 border border-white/10 hover:bg-gradient-to-r hover:from-[#833ab4] hover:via-[#fd1d1d] hover:to-[#fcb045] hover:border-transparent hover:scale-105 text-gray-300 hover:text-white transition-all duration-300 text-sm font-semibold shadow-lg">
      ...
      {/* MODAL OVERLAY */}
      {modalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-xl animate-[fade-in-up_0.3s_ease-out]" onClick={() => setModalOpen(false)}>
          <div className="w-full max-w-lg p-8 rounded-[2rem] premium-glass shadow-2xl relative max-h-[90vh] overflow-y-auto no-scrollbar" onClick={e => e.stopPropagation()}>
            ...
            <h2 className="text-2xl font-bold text-white mb-8">Join {selectedEvent}</h2>

            {submitted ? (
              <div className="text-center py-12">
                <div className="w-16 h-16 rounded-full border border-white/20 bg-white/5 text-white flex items-center justify-center text-2xl mx-auto mb-6">✓</div>
                <h3 className="text-xl font-bold text-white mb-2">Application Received</h3>
                <p className="text-gray-400 text-sm mb-8">We will review your application and follow up via email.</p>
                <button className="w-full py-4 bg-white text-black font-semibold rounded-full shimmer-btn transition-transform hover:scale-[1.02] shadow-xl" onClick={() => setModalOpen(false)}>Return to site</button>
              </div>
            ) : (
              <form onSubmit={async (e) => { 
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-medium text-gray-400 ml-1">Name of the Candidate</label>
                  <input required placeholder="Jane Doe" className="w-full p-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-white/30 focus:outline-none transition-colors text-sm" />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium text-gray-400 ml-1">Enrollment No.</label>
                    <input required placeholder="e.g. 123456789" className="w-full p-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-white/30 focus:outline-none transition-colors text-sm" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium text-gray-400 ml-1">Mobile No.</label>
                    <input required type="tel" placeholder="+1 234 567 8900" className="w-full p-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-white/30 focus:outline-none transition-colors text-sm" />
                  </div>
                </div>

                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-medium text-gray-400 ml-1">Email Address</label>
                  <input required type="email" placeholder="jane@example.com" className="w-full p-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-white/30 focus:outline-none transition-colors text-sm" />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium text-gray-400 ml-1">Course</label>
                    <input required placeholder="B.Tech, BCA, etc." className="w-full p-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-white/30 focus:outline-none transition-colors text-sm" />
                  </div>
                  <div className="flex flex-col gap-1.5 relative">
                    <label className="text-xs font-medium text-gray-400 ml-1">Semester</label>
                    <select required defaultValue="" className="w-full p-3 rounded-xl bg-[#0f0f0f] border border-white/10 text-white focus:border-white/30 focus:outline-none transition-colors text-sm appearance-none">
                ...
                <div className="flex flex-col gap-1.5 relative">
                  <label className="text-xs font-medium text-gray-400 ml-1">Applying for Position</label>
                  <select required defaultValue="" className="w-full p-3 rounded-xl bg-[#0f0f0f] border border-white/10 text-white focus:border-white/30 focus:outline-none transition-colors text-sm appearance-none">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium text-gray-400 ml-1">Profile Photo</label>
                    <input name="profile_photo" required type="file" accept="image/*" className="w-full p-2.5 rounded-xl bg-white/5 border border-white/10 text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-white file:text-black hover:file:bg-gray-200 focus:outline-none transition-colors text-sm cursor-pointer" />
                  </div>

                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium text-gray-400 ml-1">Resume (PDF/DOC)</label>
                    <input name="resume" required type="file" accept=".pdf,.doc,.docx" className="w-full p-2.5 rounded-xl bg-white/5 border border-white/10 text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-emerald-500 file:text-black hover:file:bg-emerald-400 focus:outline-none transition-colors text-sm cursor-pointer" />
                  </div>
                </div>
                
                <button type="submit" className="w-full mt-4 py-4 bg-white text-black font-semibold rounded-full shimmer-btn transition-transform hover:scale-[1.02] flex items-center justify-center gap-2 shadow-xl">
                  Submit Application
      ...
      {activeAlbum && (
        <div className="fixed inset-0 z-[300] flex flex-col items-center justify-center p-4 md:p-8 lg:p-12 bg-black/95 backdrop-blur-2xl animate-[fade-in_0.2s_ease-out]" onClick={() => setActiveAlbum(null)}>
          <div className="w-full max-w-6xl flex justify-between items-center mb-6" onClick={e => e.stopPropagation()}>
            <div>
              <h2 className="text-2xl md:text-3xl font-bold text-white">{activeAlbum.title}</h2>
              <p className="text-gray-400 text-sm mt-1">{activeAlbum.images.length} photos in gallery</p>
            </div>
            <button className="text-white hover:text-gray-300 p-2.5 rounded-full bg-white/10 hover:bg-white/20 transition-colors border border-white/10" onClick={() => setActiveAlbum(null)} aria-label="Close album">✕</button>
          </div>
          ...
          <div className="w-full max-w-6xl flex-grow overflow-y-auto no-scrollbar rounded-2xl" onClick={e => e.stopPropagation()}>
            <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
              {activeAlbum.images.map((img, idx) => (
                <div key={idx} className="break-inside-avoid relative rounded-xl overflow-hidden border border-white/10 group cursor-pointer shadow-xl" onClick={() => { setZoomedImage(img); setZoomLevel(1); }}>
      ...
      {zoomedImage && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center bg-black/98 backdrop-blur-3xl" onClick={() => setZoomedImage(null)}>
      ...
      {/* LOGO VIEW MODAL */}
      {viewingLogo && logoUrl && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/80 backdrop-blur-xl animate-[fade-in-up_0.3s_ease-out]" onClick={() => setViewingLogo(false)}>
          <button className="absolute top-6 right-6 text-white hover:text-gray-300 transition-colors text-2xl z-10" onClick={() => setViewingLogo(false)} aria-label="Close logo view">✕</button>
